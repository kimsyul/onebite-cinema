...
export default function Page() {
  const { id, posterImgUrl, title, releaseDate, genres, runtime, company, subTitle, description } = mockData;
  ...
  return (
    <div className={style.container}>
      <div className={style.img_container} style={{ backgroundImage: `url('${posterImgUrl}')` }}>
        <img src={posterImgUrl} />
      </div>
      <div className={style.title}>{title}</div>
      <div className={style.info}>
        {releaseDate} / {joinedGenres} / {runtime}분
      </div>
      <div>{company}</div>
